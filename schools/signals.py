...
def _compute_changes(instance, created):
...
def _log_action(instance, created=False, forced_action=None):
    if getattr(instance, '_audit_skip_signals', False):
        return

    context = get_audit_context()
    action = context['action'] or ('CREATE' if created else 'UPDATE')
    if forced_action:
    ...
    FinancialAuditLog.log_action(
        ...
        user=context['user'],
        action=action,
        changes=_compute_changes(instance, created),
        extra_data=context['extra_data'],
        notes=context['notes'],
    )


def _capture_old_snapshot(sender, instance, **kwargs):
    if getattr(instance, '_audit_skip_signals', False):
        return
...
def capture_pre_save_snapshot(sender, instance, **kwargs):
    _capture_old_snapshot(sender, instance, **kwargs)
...
def log_post_save(sender, instance, created, **kwargs):
    _log_action(instance, created=created)
